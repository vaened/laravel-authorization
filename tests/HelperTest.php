...
class HelperTest extends TestCase
{
    public function test_the_authorizer_method_returns_an_instance_of_the_configured_driver(): void
    {
        ...
        $this->assertInstanceOf(CacheAuthorizer::class, $this->app->make(Helper::class)->authorizer());

        $this->configDriver(Drivers::DATABASE);
        $this->assertInstanceOf(DatabaseAuthorizer::class, $this->app->make(Helper::class)->authorizer());
    }

    public function test_the_authenticated_method_returns_an_authenticated_user_instance(): void
    {
        $this->assertNull($this->app->make(Helper::class)->authenticated());
        $user = $this->user();
        $this->actingAs($user);
        $this->assertInstanceOf(Authorizable::class, $this->app->make(Helper::class)->authenticated());
    }
    ...
    public function test_the_method_to_exclude_authorizations_works_correctly(): void
    {
        ...
        $allowed = $this->app->make(Helper::class)->except($permissions, [$edit->getSecretName()]);
